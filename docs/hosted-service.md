...
- In Codex mode, the hosted child receives only a dedicated service-to-broker token. The broker
  owns encrypted `auth.json`, serializes refresh-token use, and never returns that credential. An
  optional default credential lets this authenticated service auto-enroll newly installed
  repositories; Actions/OIDC callers can never use that path.
