...
rules, and generated review content are sent to the model provider selected in the dashboard. For
`codex-broker`, they pass through the operator's broker to the official Codex service; the hosted
service never receives the broker's ChatGPT credential. GitHub receives the resulting review. No
repository content is sold or used to train a separate model by this service.
...
- GitHub webhook delivery IDs are retained for at most seven days and capped at 10,000 records for
  replay protection.
